...
# 1.3 Почему количество документов меньше количества вхождений? Ответ: Кол-во документов меньше кол-ва вхождений, так как в одном документе определенное слово может встречаться несколько раз.
```
...
```
# 3.4 Ответ: В поле (в каждом из полей) Слово лексико-грам. поиска нужно указывать лишь одну конкретную лексему или словоформу и/или их признаки. В то время как в поле Поиск точных форм может осуществляться поиск с указанием нескольких слов. Соответственно, если в поле Слово лексико-грам. поиска набрать 'баба с возу', запрос будет пустой, т.к. мы ищем 3 слова. 
```
...
```
# 6.1 Лемм со счетной формой: шар, ряд
```
